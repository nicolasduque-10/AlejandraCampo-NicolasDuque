...
# Formato de la entrada: - las letras proposionales seran: 1, 2, 3, 4, 5;
#                        - la negación de n es "CULPABLE n"  
...
def dibujar_tablero(f, n):
    ...
    direcciones = {}
    direcciones[1] = [0.165, 0.745]
    direcciones[2] = [0.5, 0.745]
    direcciones[3] = [0.835, 0.745]
    direcciones[4] = [0.255, 0.25]
    direcciones[5] = [0.745, 0.25]
    
    # Añadiendo los presuntos culpables
    Ernesto = AnnotationBbox(imagebox1, direcciones[1], frameon=False)
    axes.add_artist(Ernesto)

    Deductor = AnnotationBbox(imagebox2, direcciones[2], frameon=False)
    axes.add_artist(Deductor)
    
    Berticio = AnnotationBbox(imagebox3, direcciones[3], frameon=False)
    axes.add_artist(Berticio)
    
    Armando = AnnotationBbox(imagebox4, direcciones[4], frameon=False)
    axes.add_artist(Armando)

    Carnicero = AnnotationBbox(imagebox5, direcciones[5], frameon=False)
